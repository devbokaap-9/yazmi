...
import { editRoleData, getRolesData, getRolesDataLevelWise, getPermissions } from "../../../actions/Role";
...
const EditRole = (props) => {
  ...
  const {singleRoleData} = useSelector(
    (state) => state.rolesData && state.rolesData
  );

  if(singleRoleData){
    singleRoleData.permissions.map((data) => {
      if(data.appname === "user"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            userIdArray.push(fianldata.id)
          })
        })
      }
      if(data.appname === "federal"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            federalIdArray.push(fianldata.id)
          })
        })
      }
      if(data.appname === "region"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            regionIdArray.push(fianldata.id)
          })
        })
      }
      if(data.appname === "woreda"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            woredaIdArray.push(fianldata.id)
          })
        })
      }
      if(data.appname === "school"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            schoolIdArray.push(fianldata.id)
          })
        })
      }
      if(data.appname === "role"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            roleIdArray.push(fianldata.id)
          })
        })
      }
      if(data.appname === "form"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            formIdArray.push(fianldata.id)
          })
        })
      }
      if(data.appname === "course"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            courseIdArray.push(fianldata.id)
          })
        })
      }
      if(data.appname === "learner"){
        data.permissions.map((subdata) => {
          subdata.permissions.map((fianldata) => {
            learnerIdArray.push(fianldata.id)
          })
        })
      }
      // if(data.appname === "resource"){
      //   data.permissions.map((subdata) => {
      //     subdata.permissions.map((fianldata) => {
      //       resourceIdArray.push(fianldata.id)
      //     })
      //   })
      // }
      // if(data.appname === "announcements"){
      //   data.permissions.map((subdata) => {
      //     subdata.permissions.map((fianldata) => {
      //       announcementIdArray.push(fianldata.id)
      //     })
      //   })
      // }
    })
  ...
  useEffect(() => {
    if(singleRoleData){
      dispatch(getRolesDataLevelWise(singleRoleData.level))
      setUserArray(userIdArray)
      setFederalArray(federalIdArray)
      setRegionArray(regionIdArray)
      setWoredaArray(woredaIdArray)
      setSchoolArray(schoolIdArray)
      setRoleArray(roleIdArray)
      setFormArray(formIdArray)
      setCourseArray(courseIdArray)
      // setResourceArray(resourceIdArray)
      // setAnnouncementArray(announcementIdArray)
      setLearnerArray(learnerIdArray)
    }
  },[singleRoleData])

  const editData = useSelector(
    (state) => state.rolesData.editData
  );
  ...
  const handleEditRole = (values) => {
    ...
    // {
    if(values.userData.length !== 0 || values.federalData.length !== 0 || values.regionData.length !== 0 || values.woredaData.length !== 0 || values.schoolData.length !== 0 || values.roleData.length !== 0 || values.formData.length !== 0 || values.courseData.length !== 0 || values.learnerData.length !== 0)
    {
      ...
      let data =  {
        ...save_role,...values,...permissionData
      }

      dispatch(editRoleData(data))
    }
    else{
  ...
  useEffect(() => {
    ...
    if (editData.id && props.show && btnClickFlag && error.errorsData === '') {
      props.closeModal();
      setBtnClickFlag(false)
      setLoading(false);
      toast.success('Role Updated');
      dispatch(getRolesData(!props.groupByLevel));
    }
    else{
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        <Formik
          initialValues={{
            title: singleRoleData.title,
            level: singleRoleData.level,
            parent: singleRoleData.parent,
            userData: userArray,
            federalData: federalArray,
            regionData: regionArray,
            woredaData: woredaArray,
            schoolData: schoolArray,
            roleData:roleArray,
            formData: formArray,
            courseData:courseArray,
            resourceData:resourceArray,
            announcementData:announcementArray,
            learnerData : learnerArray
          }}
              ...
              <Form>
                <div className="body_head">
                  ...
                  <div className="uf-single">
                    ...
                    <div className="uf-field">
                      <select className="search-yazmi" onChange={(e) => {levelChange(e)}} name="level" defaultValue={values.level}>
