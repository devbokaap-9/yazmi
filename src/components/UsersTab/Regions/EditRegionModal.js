...
import { UpdateRegionData, getRegionsData } from "../../../actions/Region";
...
const EditRegionModal = (props) => {
  ...
  const regionResData = useSelector(
    (state) =>
      state.regionData.updateRegionData
  );
  ...
  const handleSave = () => {
    ...
    dispatch(UpdateRegionData(save_region));
  };
        ...
        <Modal.Body>
          ...
          >
            <Tab eventKey="Basic" title="Basic data">
              <Formik
                initialValues={{ usernames: singleRegionData.name, idnumbers: singleRegionData.identification_number,latitude:singleRegionData.geolocation && singleRegionData.geolocation.split(',')[0],longitude:singleRegionData.geolocation && singleRegionData.geolocation.split(',')[1] }}
            ...
            <Formik
                initialValues={{ addisababa: singleRegionData.distance_from_addis_abba,centerrigion:singleRegionData.distance_from_central_region,centerzone:singleRegionData.distance_from_central_zone }}
                innerRef={form1Ref}
                onSubmit={handleSave}
              >
              {({ values }) => {
