...
import ViewRegionUerModal from "./ViewRegionUerModal";
...
import { getRegionsData, getSingleRegionUser, singleDataForRegionUser } from "../../../../actions/Region";
...
const headRows = [
  ...
  { id: "region", numeric: true, disablePadding: false, label: "Region Name" },
  { id: "woreda", numeric: true, disablePadding: false, label: "Woreda Name" },
...
const EnhancedTable = (props) => {
  ...
  const [showViewRegionUser, setShowViewRegionUser] = useState(false);
...
// get data
  ...
  // single region all user
  const { getSingleRegionUserData } = useSelector((state) => state.regionData);
  var { count, results } = getSingleRegionUserData;
  ...
  useEffect(() => {
    ...
    if (results != undefined && results.length !== 0) {
      var data = results.map((data) =>
        createData(
          data && data.id,
          data.profile.first_name && data.profile.first_name.charAt(0).toUpperCase() + data.profile.first_name.slice(1),
          ...
          data.profile.status,
          data.profile.region_name && data.profile.region_name.charAt(0).toUpperCase() + data.profile.region_name.slice(1),
          data.profile.woreda_name && data.profile.woreda_name.charAt(0).toUpperCase() + data.profile.woreda_name.slice(1),
          data && data.profile && data.profile.id
        )
      );
      setRows(data);
      setShowEditRegionUser(false)
      setShowViewRegionUser(false)
      setUserData(data)
  ...
  // setTimeout(function(){
  //   var isExit = document.getElementsByClassName('MuiTableCell-head');
  //   if(isExit[0] && isExit[0].childNodes && isExit[0].childNodes[0].classList && isExit[0].childNodes[0].classList[6] === "Mui-checked"){
  //     props.setCheck(true)
  //   }
  //   else{
  ...
  const showViewRegionUserModal = (id) => {
    setViewLoading(true)
    if(id && params.id){
      let ViewId = {
        regionId: params.id,
      ...
      dispatch(singleDataForRegionUser(ViewId))
    }
  }

  const closeViewRegionUserModal = () => {
    setShowViewRegionUser(false);
  };

  const showEditRegionUserModal = (id) => {
    setEditLoading(true)
    if(id && params.id){
      let ViewId = {
        regionId: params.id,
  ...
  const closeEditRegionUserModal = () => {
    setShowEditRegionUser(false);
  };
  ...
  return (
    <>
                    ...
                    return (
                      ...
                      <TableCell
                        ...
                        <TableCell
                          ...
                          scope="row"
                          padding="none"
                          onClick={() => {showViewRegionUserModal(row.id)}}
                      ...
                      <TableCell align="left">
                        <div className={props.check ? "action_icon invisible" : "action_icon"} >
                          {(checkPermission('custom_region_data_delete')) || checkPermission('custom_region_data_all')
                          ?
                          ...
                          :
                            ...
                            {(checkPermission('custom_region_data_edit')) || checkPermission('custom_region_data_all')
                            ?
                              <span
                                className="datatable_icon edit_icon" onClick={()=>{showEditRegionUserModal(row.id)}}>
                                <svg
                                  width="16"
                                  height="16"
                                  viewBox="0 0 16 16"
      ...
      <DeleteModal deleteRole="regionUser" name={userName} id={getId} show={showDelete} closeModal={closeDeleteModal} /> 
     <SuspendModal deleteRole="regionUser" name={userName} id={profileId} show={showSuspend} closeModal={closeSuspendModal} actionName="suspend"/>
      ...
      <EditUserModal name="Other" show={showEditRegionUser} closeModal={closeEditRegionUserModal} />
      <ViewRegionUerModal
        id={getId}
        show={showViewRegionUser}
        closeModal={closeViewRegionUserModal}
      />
      <ToastContainer
        position="top-center"
        autoClose={2000}
