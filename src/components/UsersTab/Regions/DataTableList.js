...
import { getRegionsData,getSingleRegionData } from "../../../actions/Region";
...
const headRows = [
  { id: "name", numeric: false, disablePadding: true, label: "Region Name" },
  { id: "calories", numeric: false, disablePadding: false, label: "Users" },
  { id: "fat", numeric: false, disablePadding: false, label: "Woredas" },
  { id: "carbs", numeric: false, disablePadding: false, label: "Schools" },
...
const EnhancedTable = (props) => {
  ...
  const [showViewRegion, setShowViewRegion] = useState(false);
...
// get data
  ...
  // single data
  const singleRegionData = useSelector((state) => state.regionData.singleRegionData);
  ...
  useEffect(() => {
    ...
    if (results != undefined) {
      var data = results.map((data) =>
        createData(
          data.id,
          data && data.name && data.name.charAt(0).toUpperCase() + data.name.slice(1),
          data.users.length,
          data.no_of_woredas,
          data.no_of_schools,
          data.id
  ...
  const showViewRegionModal = (model_id) => {
    setGetId(model_id)
    setViewLoading(true)
    dispatch(getSingleRegionData(model_id))
  };

  const closeViewRegionModal = () => {
    setShowViewRegion(false);
  };
  const showEditRegionModal = (model_id) => {
    setViewId(model_id)
    setEditLoading(true)
    dispatch(getSingleRegionData(model_id))
  };
  ...
  const requestSearch = (searchedVal) => {
    const filteredRows = userData.filter((row) => {
      ...
      return (row.name && row.name.toLowerCase().includes(newSearchedVal)) || row.calories.toString().includes(newSearchedVal) || row.fat.toString().includes(newSearchedVal) || row.carbs.toString().includes(newSearchedVal);
    });
  ...
  return (
    <>
                    ...
                    return (
                      ...
                      <TableCell
                        ...
                        <TableCell
                          ...
                          scope="row"
                          padding="none"
                          onClick={() => {showViewRegionModal(row.id)}}
                      ...
                      <TableCell align="left">
                        <div className={props.check ? "action_icon invisible" : "action_icon"}>
                          ...
                          ?
                            <span className="datatable_icon delete_icon" onClick={() => {showDeleteModal(row.id)}}>
                              <svg
                                width="11"
                                height="14"
                                viewBox="0 0 11 14"
                          ...
                          ?
                            <span className="datatable_icon medium_icon" onClick={() => {showSuspendModal(row.id)}}>
                              <svg
                                width="9"
                                height="12"
                                viewBox="0 0 9 12"
                          ...
                          :
                            ...
                            ?
                              <span
                                ...
                                onClick={()=>{showEditRegionModal(row.id)}}
                              >
                                <svg
                                  width="16"
                                  height="16"
      ...
      <ViewRegionModal
        id={getId}
        show={showViewRegion}
        closeModal={closeViewRegionModal}
      />
      <EditRegionModal
        id={viewId}
        show={showEditRegion}
        closeModal={closeEditRegionModal}
      />
       <DeleteModal name="region" id={deleteId} show={showDelete} closeModal={closeDeleteModal} />
     <SuspendModal deleteRole="region" name="region" id={deleteId} show={showSuspend} closeModal={closeSuspendModal} actionName="suspend"/>
       <div className={editLoading || viewLoading ? "loader_div" : ""}>
       <ClipLoader color={color} className="loader"
