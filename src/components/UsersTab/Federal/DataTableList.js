...
import { getFederalData, getSingleFederalData, getSingleFederalUserData } from "../../../actions/Federal";
...
import ViewFederalUserModal from "./ViewFederalUserModal";
...
const EnhancedTable = (props) => {
  ...
  const [showFederalUser, setShowFederalUser] = useState(false);
  const [editFederalUser, setEditFederalUser] = useState(false);
  ...
  // single fedral data 
  const singlefederal  = useSelector((state) => state.federalData && state.federalData.singlefederal);
  
  var findOne = singlefederal.results &&  singlefederal.results

  // single federal single user
  const single_user = useSelector((state) => state.federalData && state.federalData.singlefederaluserdata);

  useEffect(() => {
    if(single_user){
      if(!editLoading){
        setViewLoading(false)
        setShowFederalUser(true)
      }
      if(!viewLoading){
        setEditLoading(false)
        setEditFederalUser(true)
      }
    }
  }, [single_user])
  ...
  function createData(id, name, calories, fat, carbs , region, woreda,protein) {
    return { id, name, calories, fat, carbs, region, woreda,protein };
  }

  useEffect(() => {
    userContext.setFrmLdng(false)
    if (singlefederal.results != undefined) {
      var data = singlefederal.results.map((data) =>
        createData(
          data.id,
          data.profile && data.profile.first_name.charAt(0).toUpperCase() + data.profile.first_name.slice(1),
          // data.profile && data.profile.username.charAt(0).toUpperCase() + data.profile.username.slice(1),
          ...
          data.profile && data.profile.status,
          data.profile && data.profile.region_name.charAt(0).toUpperCase() + data.profile.region_name.slice(1),
          data.profile && data.profile.woreda_name.charAt(0).toUpperCase() + data.profile.woreda_name.slice(1),
      ...
      setUserData(data)
      props.count(singlefederal.results.length)
    }
  }, [singlefederal]);
  ...
  const showViewFederalUserModal = (id) => {
    setViewLoading(true)
    if(id){
      dispatch(getSingleFederalUserData(id,FederalId))
    }
  }
  ...
  const requestSearch = (searchedVal) => {
    const filteredRows = userData.filter((row) => {
      ...
      return (row.name && row.name.toLowerCase().includes(newSearchedVal)) || (row.calories && row.calories.toLowerCase().includes(newSearchedVal)) || (row.fat && row.fat.toLowerCase().includes(newSearchedVal)) || (row.carbs && row.carbs.toLowerCase().includes(newSearchedVal)) || (row.region && row.region.toLowerCase().includes(newSearchedVal)) || (row.woreda && row.woreda.toLowerCase().includes(newSearchedVal))
      //  || row.carbs.toLowerCase().includes(searchedVal)
    });
  ...
  return (
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell
                        ...
                        scope="row"
                        padding="none"
                        onClick={() => {showViewFederalUserModal(row.id)}}
                      ...
                      <TableCell align="left">
                        ...
                        {(checkPermission('custom_federal_data_delete')) || checkPermission('custom_federal_data_all')
                        ?
                          <span className="datatable_icon delete_icon" onClick={() => {showDeleteModal(row.id,row.name)}}>
                            <svg
                              width="11"
                              height="14"
                              viewBox="0 0 11 14"
                        ...
                        ?
                          <span className="datatable_icon medium_icon" onClick={() => {showSuspendModal(row.protein,row.name)}}>
                            <svg
                              width="9"
                              height="12"
                              viewBox="0 0 9 12"
                        ...
                        :
                          ...
                          {(checkPermission('custom_federal_data_edit')) || checkPermission('custom_federal_data_all')
                          ?
                            <span className="datatable_icon edit_icon" onClick={()=>{showEditFederalUserModal(row.id)}}>
                              <svg
                                width="16"
                                height="16"
                                viewBox="0 0 16 16"
    ...
    <div className={editLoading || viewLoading ? "loader_div" : ""}>
     ...
     <DeleteModal deleteRole="federalsUser" name={userName} id={deleteId} show={showDelete} closeModal={closeDeleteModal} />
     <SuspendModal deleteRole="federalsUser" name={userName} id={profileId} show={showSuspend} closeModal={closeSuspendModal} actionName="suspend"/>
    <EditUserModal name="Other" show={editFederalUser} closeModal={closeEditFederalUserModal} />
     <ViewFederalUserModal show={showFederalUser} closeModal={closeViewFederalUserModal} />
     <ToastContainer
        position="top-center"
        autoClose={2000}
