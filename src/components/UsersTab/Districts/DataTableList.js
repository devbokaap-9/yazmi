...
import { getWoredasData,getWoredaData } from "../../../actions/Woreda";
...
const headRows = [
  { id: "name", numeric: false, disablePadding: true, label: "Woreda Name" },
  { id: "calories", numeric: true, disablePadding: false, label: "Users" },
  { id: "fat", numeric: true,  disablePadding: false, label: "School" },
  { id: "carbs", numeric: false, disablePadding: false, label: "Assigned to" },
...
const EnhancedTable = (props) => {
  ...
  const { woredaData } = useSelector((state) => state.woredaData);
  
  var { count, results } = woredaData;

  // single data
  const singleWoredaData = useSelector((state) => state.woredaData.singleWoredaData);
  useEffect(() => {
    if(singleWoredaData){
      if(!editLoading){
  ...
  const emptyRows =
    ...
    const requestSearch = (searchedVal) => {
      const filteredRows = userData.filter((row) => {
        ...
        return (row.name && row.name.toLowerCase().includes(newSearchedVal)) || row.calories.toString().includes(newSearchedVal) || row.fat.toString().includes(newSearchedVal) || (row.carbs && row.carbs.toLowerCase().includes(newSearchedVal));
      });
  ...
  return (
    ...
    <div className={classes.root}>
                    ...
                    >
                      ...
                      <TableCell align="left">
                        <div className={props.check ? "action_icon invisible" : "action_icon"}>
                          ...
                          ?
                            <span className="datatable_icon edit_icon" onClick={() => {showEditDistrictModal(row.id)}}>
                              <svg
                                width="16"
                                height="16"
                                viewBox="0 0 16 16"
      ...
      <ViewDistrictModal show={showViewDistrict} closeModal={closeViewDistrictModal} />
      <EditDistrictModal show={showEditDistrict} closeModal={closeEditDistrictModal} />   
      <DeleteModal name="woreda" id={delteId} show={showDelete} closeModal={closeDeleteModal} /> 
     <SuspendModal deleteRole="woreda" name="woreda" id={delteId} show={showSuspend} closeModal={closeSuspendModal} actionName="suspend"/>
