...
import { deleteWoredaData } from "../../../actions/Woreda"; 
...
const DeleteSchoolModal = (props) => {
    ...
    useEffect(() => {
        if(deleteRole === "woredaUserDelete"){
            ...
            else if(deleteworeda.status === 204 && btn === true){
                props.closeModal()
                notify()
                history.push("/woreda");
                setBtn(false)
    ...
    const handleDelete = () => {
        if(deleteRole === "woredaUserDelete"){
            setLoading(true)
            dispatch(removeErrorData)
            dispatch(deleteWoredaData(locationId))
            setBtn(true)
        }
    }
    ...
    return (
        <>
            ...
            {
                deleteworeda.status === 204 ? <ToastContainer
                position="top-center"
                autoClose={2000}
