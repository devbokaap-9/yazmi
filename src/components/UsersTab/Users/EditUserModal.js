...
const EditUserModal = (props) => {
	...
	let singleUserData = useSelector((state) =>
		paramName === 'schools'
			? state.schoolData && state.schoolData.singleUserSchoolData
			: paramName === 'woreda'
			? state.woredaData.getWoredaSingleUserData
			: paramName === 'regions'
			? state.regionData.getSingleDataForRegion
			: federalParamName === 'federal'
			? state.federalData && state.federalData.singlefederaluserdata
			: ''
	);
	...
	useEffect(() => {
		...
		if (
			...
			// console.log(singleUserData, 'users');
			// dispatch(
			// 	dispatch(getRegionSpecificWoredasData(singleUserData.region))
			// );
	...
	useEffect(() => {
		...
		setDates(newDate);
		if (singleUserData !== undefined && singleUserData.profile) {
			dispatch(
				getRegionSpecificWoredasData(singleUserData.profile.region)
			);
		}
	}, [singleUserData]);
	...
	var FederalId =
		federalDatas &&
		federalDatas.results &&
		federalDatas.results[0] &&
		federalDatas.results[0].id;
	...
	const saveAllData = (e) => {
		...
		if (formRef.current.values.role !== '') {
			if (formRef.current.values.role.includes(',')) {
				let role = formRef.current.values.role.split(',');
				formRef.current.values.role = role[0];
				if (
					role[1] !== 'Student' &&
					role[1] !== 'student' &&
			...
			let idData = {
				id:
					singleUserData &&
					singleUserData.profile &&
					singleUserData.profile.id,
				user:
					singleUserData &&
					singleUserData.profile &&
					singleUserData.profile.user,
			};
			...
			if (
				...
				dispatch(updateProfileData(save_data));
			}
		} else {
	...
	const userResData = useSelector((state) => state.usersData.updateUserData);
	...
	useEffect(() => {
		...
		if (
			...
			if (paramName === 'schools') {
				toast.success('School User Updated');
				dispatch(schoolUserData(params.id));
			} else if (paramName === 'woreda') {
				toast.success('Woreda User Updated');
				dispatch(getWoredaUser(params.id));
			} else if (paramName === 'regions') {
				toast.success('Region User Updated');
				dispatch(getSingleRegionUser(params.id));
			} else if (federalParamName === 'federal') {
				toast.success('Federal User Updated');
							...
							<Formik
								initialValues={{
									role:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.roles[0] &&
										singleUserData.profile.roles[0].id +
											',' +
											singleUserData.profile.roles[0]
												.title,
									username:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.username,
									password:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.username,
									first_name:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.first_name,
									middle_name:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.middle_name,
									last_name:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.last_name,
									phone_number:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.phone_number,
									sex:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.sex,
									citizenship:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.citizenship,
									dob:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.dob,
								}}
								validationSchema={
									user === 'Student' || user === 'student'
							...
							<Formik
								initialValues={{
									address:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.address,
									region:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.region,
									woreda:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.woreda,
									city:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.city,
									kebele:
										singleUserData &&
										singleUserData.profile &&
										singleUserData.profile.kebele,
								}}
								validationSchema={
