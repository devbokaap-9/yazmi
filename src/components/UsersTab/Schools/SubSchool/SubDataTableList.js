...
import { getSingleSchoolUser, schoolUserData } from "../../../../actions/School";
import 'react-toastify/dist/ReactToastify.css';
...
const headRows = [
  ...
  { id: "woreda", numeric: false, disablePadding: false, label: "Woreda" },
...
const EnhancedTable = (props) => {
  ...
  const [userSchoolDeleteId, setUserSchoolDeleteId] = useState();
  const [userSchoolViewId, setUserSchoolViewId] = useState();
  ...
  const userSchoolData = useSelector((state) => state.schoolData.userSchoolData);
  const {results} = userSchoolData

  const SingleuserSchoolData = useSelector((state) => state.schoolData && state.schoolData.singleUserSchoolData);

  useEffect(() => {
    if(SingleuserSchoolData){
      if(!editLoading){
  ...
  useEffect(() => {
    ...
    if (results !== undefined && results.length !== 0) {
      var data = results.map((data) =>
        createData(
          data && data.id,
          data && data.profile.first_name && data.profile.first_name.charAt(0).toUpperCase() + data.profile.first_name.slice(1),
          ...
          data.profile.status,
          data && data.profile && data.profile.region_name && data.profile.region_name.charAt(0).toUpperCase() + data.profile.region_name.slice(1),
          data && data.profile && data.profile.woreda_name && data.profile.woreda_name.charAt(0).toUpperCase() + data.profile.woreda_name.slice(1),
  ...
  const showViewStudentModal = (id,rolename) => {
    ...
    if(id && params.id){
      ...
      setUserSchoolViewId(ViewId)
      dispatch(getSingleSchoolUser(ViewId))
    }
  }
  ...
  const showEditStudentrModal = (id,rolename) => {
    setRolename(rolename)
    setEditLoading(true)
    if(id && params.id){
      let ViewId = {
  ...
  const showDeleteModalSchool = (id,rolename,name) => {
    setShowDelete(true);
    setRolename(rolename)
    setUserName(name)
    if(id && params.id){
      ...
      setUserSchoolDeleteId(deleteId)
    }
  }
  ...
  return (
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell
                        ...
                        scope="row"
                        padding="none"
                        onClick={()=> {showViewStudentModal(row.id,row.Role)}}
                      ...
                      <TableCell align="left">
                        ...
                        {(checkPermission('custom_school_data_delete')) || checkPermission('custom_school_data_all')
                        ?                     
                          <span className="datatable_icon delete_icon" onClick={()=>{showDeleteModalSchool(row.id,row.Role,row.name)}}>
                            <svg
                              width="11"
                              height="14"
                              viewBox="0 0 11 14"
                        ...
                        {(checkPermission('custom_school_operation_suspend'))
                        ?
                        ...
                        :
                          ...
                          {(checkPermission('custom_school_data_edit')) || checkPermission('custom_school_data_all')
                          ?
                            <span className="datatable_icon edit_icon" onClick={() => {showEditStudentrModal(row.id,row.Role)}}>
                              <svg
                                width="16"
                                height="16"
                                viewBox="0 0 16 16"
    ...
    {/* View Student Modal */}
        ...
        ?
          <ViewStudentModal id={userSchoolViewId} loading={setViewLoading} show={showViewTeacher} close={closeViewStudentModal} name={rolename}/>
        :
          (rolename === "Teacher" || rolename === 'teacher')
          ?
            <ViewTeacherModal id={userSchoolViewId} loading={setViewLoading} show={showViewTeacher} close={closeViewStudentModal} name={rolename}/>
          :
            
            <ViewUserModal
            name="school"
            show={showViewTeacher}
            closeModal={closeViewStudentModal}
          />
        }
       {/* Edit Modal */}
       {rolename === "Student" || rolename === 'student' || rolename === "Teacher" || rolename === 'teacher'
        ?
          <EditUserModal name={rolename} show={showEditTeacher} closeModal={closeEditStudentModal}/>
        :
          <EditUserModal name="Other" show={showEditTeacher} closeModal={closeEditStudentModal}/>
      }
       <DeleteModal id={userSchoolDeleteId} name={userName} deleteRole="schoolUser" show={showDelete} closeModal={closeDeleteModal} />
     <SuspendModal deleteRole="schoolUser" name={userName} id={profileId} show={showSuspend} closeModal={closeSuspendModal} actionName="suspend"/>
