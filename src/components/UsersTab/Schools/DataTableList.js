...
import {getSchoolsData, getSingleSchoolData} from "../../../actions/School"
...
const headRows = [
  ...
  { id: "calories", numeric: true, disablePadding: false, label: "Users" },
  { id: "fat", numeric: false,  disablePadding: false, label: "Woreda Name" },
  { id: "carbs", numeric: false, disablePadding: false, label: "Region Name" },
...
const EnhancedTable = (props) => {
    ...
    if (results != undefined) {
      var data = results.map((data) =>
        createData(
          ...
          data.users.length,
          data && data.region_name && data.region_name.charAt(0).toUpperCase() + data.region_name.slice(1),
          data && data.woreda_name && data.woreda_name.charAt(0).toUpperCase() + data.woreda_name.slice(1),
          data.id
        )
      );
      setRows(data);
  ...
  const requestSearch = (searchedVal) => {
    const filteredRows = userData.filter((row) => {
      if(userData){
        ...
        return (row.name && row.name.toLowerCase().includes(newSearchedVal)) || row.calories.toString().includes(newSearchedVal) || (row.fat && row.fat.toLowerCase().includes(newSearchedVal)) || (row.carbs && row.carbs.toLowerCase().includes(newSearchedVal))
      }
    });
  ...
  return (
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell
                        ...
                        scope="row"
                        padding="none"
                        onClick={() => {showViewSchoolModal(row.id)}}
                      ...
                      <TableCell align="left">
                        ...
                        :
                          ...
                          ?
                            <span className="datatable_icon edit_icon" onClick={() => {showEditSchoolModal(row.id)}}>
                              <svg
                                width="16"
                                height="16"
                                viewBox="0 0 16 16"
    ...
    <ViewSchoolModal show={showViewSchool} closeModal={closeViewSchoolModal} />
    <EditSchoolModal show={showEditSchool} closeModal={closeEditSchoolModal} />
    <DeleteModal name="school" id={delteId} show={showDelete} closeModal={closeDeleteModal} />
     <SuspendModal deleteRole="school" name="school" id={delteId} show={showSuspend} closeModal={closeSuspendModal} actionName="suspend"/>
